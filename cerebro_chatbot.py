...
def pesquisar_e_resumir_web(query, pergunta_original):
    """Pesquisa na web, extrai o conteúdo e retorna para a IA principal."""
    try:
...
def gerar_resposta_com_base_no_contexto(pergunta_usuario, contexto):
    """Função central que usa o Cohere para gerar a resposta final com base em um contexto."""
...
def gerar_resposta_final(pergunta_usuario):
    """Decide qual ferramenta usar: busca interna ou pesquisa web."""
    if co is None: 
        return "Desculpe, o cliente da IA não está disponível."
    ...
    if any(trigger in entrada_formatada for trigger in triggers_pesquisa):
        ...
        if not contexto: 
            return "Erro: Nenhuma base de conhecimento (.txt) foi encontrada."
